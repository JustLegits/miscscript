...
def send_status_webhook(webhook_url, device_name, username, uid, status):
    status_map = {
        0: "Offline",
        1: "Lobby",
        2: "Playing",
    ...
    embed = {
        "title": f"User Status Update - {device_name}",
        "color": 3066993 if status == 2 else (15105570 if status == 1 else 15158332),
        "fields": [
    ...
    try:
        ...
        if response.status_code in (200, 204):
            msg("Sent status to webhook!", "ok")
        else:
            msg("Failed to send status webhook!", "err")
    except Exception:
        msg("Failed to send status webhook!", "err")
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                status_cache = {}  # Add this line to cache previous statuses
                while True:
                    for package, uid in accounts:
                        username = get_username(uid) or uid
                        prev_status = status_cache.get(uid)
                        status = robust_check_user_online(uid, prev_status)
                        ...
                        if status == 2:
                            ...
                            msg(f"{username} ({uid}) status unknown.", "warn")
                        time.sleep(3)
